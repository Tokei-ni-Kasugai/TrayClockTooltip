...
| TM-02 | Default tray menu entries | Right-click tray icon when no drift at or above the current notification threshold is known. | Menu shows `NTP: refresh`, `Notify threshold`, `Startup`, and `Exit`. |
| TM-03 | Tray NTP refresh | Right-click tray icon, choose `NTP: refresh`. | App queries the Windows Time configured NTP server again. If the query succeeds and drift is below the current notification threshold, a success notification is shown. |
...
| TM-05 | Drift tray menu status | Create or simulate Windows time drift at or above the current notification threshold, then let the app query NTP. | Tray menu shows a status-only `Time: +...s (...)` or `Time: -...s (...)` item at the top. It uses normal menu colors, does not highlight on hover, and does not close the menu when clicked. |
| TM-06 | Tray adjust item appears | With drift at or above the current notification threshold detected, right-click tray icon. | `Adjust Windows time (admin)` appears below the status-only `Time: ...` item. Selecting it starts time adjustment with administrator rights. |
| TM-07 | Adjust menu hidden below threshold | With drift below the current notification threshold after NTP query, right-click tray icon. | `Adjust Windows time (admin)` is not shown unless notification threshold is `Off`. |
| TM-08 | Advanced forced adjust menu | With drift below the current notification threshold after NTP query, hold Shift and right-click tray icon. | Tray menu shows `NTP: refresh`, `Adjust Windows time (admin)`, `Notify threshold`, `Startup`, and `Exit`. Selecting Adjust starts time adjustment with administrator rights. |
| TM-09 | No adjust while querying | While an NTP query is in progress, right-click the tray icon normally and with Shift. | `NTP: refresh` is disabled and stale `Adjust Windows time (admin)` is not shown. Threshold changes made while querying do not revive stale adjust state; the completed NTP result is evaluated with the new threshold. |
...
| TM-13 | Notification threshold menu | Open the tray menu and choose each `Notify threshold` value: `1 sec`, `3 sec`, `5 sec`, `7 sec`, `10 sec`, and `Off`. | The selected item is checked the next time the menu opens. The current process applies the new threshold immediately. The floating clock menu does not show `Notify threshold`. |
| TM-14 | Notification threshold Off menu behavior | Set `Notify threshold` to `Off`, let NTP acquisition succeed, then open the normal tray and floating menus. | Automatic drift notifications are not shown. `Adjust Windows time (admin)` is available from normal menus without holding Shift, unless an NTP query is in progress or no NTP time has been obtained. |

## Startup Registration

| ID | Case | Steps | Expected |
| --- | --- | --- | --- |
| SU-01 | Install for this user | Choose tray menu `Startup` -> `Install for this user`. | App copies the current EXE to `%LOCALAPPDATA%\Programs\TrayClockTooltip\TrayClockTooltip.exe`, verifies that the copied file matches the current EXE, copies release README files when they exist next to the source EXE, writes that path plus the current notification threshold argument when needed to `HKCU\Software\Microsoft\Windows\CurrentVersion\Run\TrayClockTooltip`, starts the installed EXE after the current process exits, shows an install success notification from the installed EXE, and does not show UAC. |
| SU-02 | Register current EXE at startup | Choose tray menu `Startup` -> `Add this EXE to startup`. | App writes the current EXE path plus the current notification threshold argument when needed to `HKCU\Software\Microsoft\Windows\CurrentVersion\Run\TrayClockTooltip` and shows a success notification. UAC is not shown. |
...
| SU-07 | Retired: old install handoff case | This case was retired. | The previous case duplicated install/update handoff behavior and no longer matched the current menu disabled-state rules. Use SU-10, SU-10A, SU-11, and SU-12 for install handoff coverage. |
...
| SU-10 | Update installed EXE while installed instance is running | Start the installed EXE, then run a different EXE with `--install-prompt` and choose install from the prompt. | App asks the running installed instance to exit normally, waits briefly, then replaces the installed EXE. If the running instance cannot exit, install fails without force-killing it. |
...
| SU-11 | Restore startup to installed EXE | Make the installed EXE match the current EXE. Start another EXE, choose `Startup` -> `Remove startup registration`, then choose `Startup` -> `Add this EXE to startup` so startup points to the other EXE. Open the tray menu again and choose `Startup` -> `Install for this user`. | `Install for this user` is enabled. Selecting it does not need to replace the installed EXE, but it rewrites startup registration to `%LOCALAPPDATA%\Programs\TrayClockTooltip\TrayClockTooltip.exe`, starts the installed EXE after the current process exits, and shows success. |
| SU-12 | Delete source folder after install handoff | Copy the EXE to any temporary folder, start that EXE, and choose `Install for this user`. After startup is handed off to the installed EXE, delete the original temporary folder. | The original temporary folder can be deleted while the installed EXE is running. The installed EXE does not keep the source folder as its current directory. |
| SU-13 | Startup threshold inheritance | Register startup with `--notify-threshold=5`, then start another EXE without a threshold argument. | The running app uses the registered `5 sec` threshold without creating a separate settings file or registry key. |
| SU-14 | Threshold change preserves registered path | Register startup to an installed EXE with `--notify-threshold=5`, start a different portable EXE, then change `Notify threshold` to `3 sec`. | The portable process immediately uses `3 sec`. The startup registration keeps the installed EXE path and changes only the argument to `--notify-threshold=3`. |
| SU-15 | Threshold change re-evaluates current offset | Acquire NTP time while known drift is above the current threshold, then raise the threshold above that drift. Also acquire NTP time while known drift is below the current threshold, then lower the threshold below that drift. | The tray/floating `Adjust Windows time (admin)` availability is recalculated immediately from the latest known NTP offset. It disappears when the new threshold is above the offset and appears when the new threshold is below the offset. Lowering the threshold into a drift state also shows the drift notification. No NTP re-query or app restart is required. |
...
| FM-02 | Default floating menu entries | Right-click floating clock when no drift at or above the current notification threshold is known and notification threshold is not `Off`. | Menu shows `Close`, separator, `Exit`. |
| FM-03 | Floating menu has no NTP refresh | Right-click floating clock when no drift at or above the current notification threshold is known. | `NTP: refresh` is not shown. |
| FM-04 | Floating menu during NTP query | Open floating menu while NTP query is in progress. | `NTP: refresh` is not shown. |
| FM-05 | Floating adjust menu appears on drift | With drift at or above the current notification threshold detected, right-click floating clock. | `NTP: refresh` is hidden and `Adjust Windows time (admin)` is shown. |
| FM-06 | Floating menu does not show drift text | With drift at or above the current notification threshold detected, right-click floating clock. | Floating menu does not show `Time: ...`. |
| FM-07 | Advanced forced floating adjust menu | With drift below the current notification threshold after NTP query, hold Shift and right-click floating clock. | Floating menu shows `Adjust Windows time (admin)`, separator, `Close`, separator, and `Exit`. Selecting Adjust starts time adjustment with administrator rights. |
...
| NT-08 | Portable startup visibility notification | Run from a non-installed EXE when no installed EXE exists and startup registration does not exist. Let startup NTP acquisition succeed with drift below the current notification threshold. | A custom notification confirms that TrayClockTooltip is running and suggests hovering the tray icon. If an installed EXE exists, startup is registered, NTP fails, or drift reaches the threshold, this startup visibility notification is not shown. |

## Drift Notification And Adjustment

| ID | Case | Steps | Expected |
| --- | --- | --- | --- |
| AD-01 | Drift notification appears | Create or simulate Windows time drift at or above the current notification threshold, then query NTP. | Custom notification appears unless notification threshold is `Off`. It states Windows time differs by the drift amount and app clock uses NTP time. |
| AD-01A | Drift notification threshold Off | Set `Notify threshold` to `Off`, create or simulate Windows time drift, then query NTP automatically at startup, logon, or unlock. | No automatic drift notification is shown. The app clock still uses the acquired NTP offset and adjustment remains available from normal menus after successful NTP acquisition. |
...
| NF-04 | Notification language | Trigger each app notification in a Japanese Windows UI environment and in a non-Japanese Windows UI environment. | Notification text is Japanese when the Windows UI language is Japanese. Notification text is English in all other UI languages. Menus, logs, and install-prompt message boxes remain English. |
...
| OQ-06 | Manual refresh notification below threshold | Manual refresh with drift below the current notification threshold shows a success notification. | This gives feedback for an explicit user action while keeping startup/session success quiet. |
...
- The build succeeded and produced `dist\TrayClockTooltip.exe`; `git diff --check` also passed.
- BS-02 and BS-03 were run by Codex. Existing `TrayClockTooltip` processes were stopped first, the built EXE launched successfully, and the launched process was stopped afterward.
- Notification threshold logic was rechecked by source trace: command-line `--notify-threshold` takes priority, startup registration is read as a fallback, and unsupported values fall back to the default 1 second threshold.
- Threshold menu wiring was rechecked by source trace: the tray menu owns `Notify threshold`, checked state follows the current value, and the floating menu does not expose the threshold menu.
- Threshold persistence was rechecked by source trace: changing the threshold updates the current process immediately and, when startup registration exists, rewrites only the registered command argument while preserving the registered EXE path.
- Threshold `Off` behavior was rechecked by source trace: automatic drift notifications are suppressed, NTP-based app time remains active after successful acquisition, and normal menus can expose adjustment without Shift after NTP time is available.
- Install README copy was rechecked by source trace: `README.txt` and `README.ja.txt` are copied to the install folder when they exist next to the source EXE, and missing README files do not fail installation.
