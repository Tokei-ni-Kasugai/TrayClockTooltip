...
- メニューとログは英語で統一し、通知本文だけWindows UI言語が日本語の場合に日本語へ切り替えます。
...
- 表示フォーマットはWindowsの短い日付形式と短い時刻形式に従い、秒が含まれない時刻形式でも秒を補完します。
...
- 通知は独自ウィンドウで表示し、Windowsモードの配色に追従します。
- 通知本文は `NotificationText` に集約し、日本語UI環境では日本語、それ以外では英語を使用します。
- メニュー、ログ、開発用 `--install-prompt` のメッセージボックスは英語のままです。
- インストール先以外からの初回ポータブル起動で、インストール先EXEもStartup登録も存在しない場合は、起動時NTP取得に成功し、ずれ通知が出ないときだけ起動確認通知を出します。これは、トレイアイコンが隠れて起動が分かりにくいケースの補助です。

## NTPずれと時刻調整

- 現在の通知しきい値以上のずれを検出した場合、独自通知を表示し、トレイ/フローティングメニューに `Adjust Windows time (admin)` を表示します。
- 通知しきい値は `1 sec`、`3 sec`、`5 sec`、`7 sec`、`10 sec`、`Off` から選択できます。`Off` は自動のずれ通知を抑止しますが、NTP基準のアプリ時計は維持します。
- 既知のずれが通知しきい値未満でも、Shiftを押しながらトレイアイコンまたはフローティング時計を右クリックすると、上級操作として時刻調整メニューを表示します。
- しきい値変更は最新の既知NTPオフセットへ即時反映し、Startup登録がある場合は登録済みEXEパスを維持したまま起動引数だけを書き換えます。
...
- `Install for this user` は、現在のEXEを `%LOCALAPPDATA%\Programs\TrayClockTooltip\TrayClockTooltip.exe` にコピーし、コピー結果をバイナリ比較で検証し、その配置先をHKCU Runへ登録します。現在のEXEが配置先と異なる場合は、現在のEXEを終了して配置先EXEを起動します。配置先EXEは `--wait-for-parent` で起動元終了を待ち、起動後にインストール成功通知を表示します。
...
- リポジトリ向けREADMEは `README.md` の英語版のみです。日本語のユーザー向け説明は配布用の `RELEASE_README.ja.md` とサイトで管理します。
...
- Release ZIPに含める変更履歴は `CHANGELOG.md` と `RELEASE_CHANGELOG.ja.md` から、`CHANGELOG.txt` と `CHANGELOG.ja.txt` として生成します。
- 配布用READMEには、ユーザーが実行・操作するための情報だけを載せます。ビルド手順、テストケース、開発用補助オプション、Source ZIP内の検証ファイルなど、開発者向けの情報は含めません。
