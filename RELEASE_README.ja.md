...
Windows標準のエクスプローラーでダウンロードしたZIPを展開し、展開後のEXE起動がブロックされる場合は、展開前にZIPファイルのプロパティを開き、`許可する` にチェックを入れてください。7-Zipで展開する場合は通常不要です。
...
- Windows時計とのずれが現在の通知しきい値以上の場合に通知。
- 通知しきい値は `1 sec`、`3 sec`、`5 sec`、`7 sec`、`10 sec`、`Off` から選択可能。
- 通知本文は日本語環境では日本語、日本語以外では英語で表示。
- 未インストールかつ自動起動未登録のポータブル初回起動では、起動確認通知を表示する場合があります。
...
- 日付はWindowsの短い日付形式、時刻はWindowsの短い時刻形式に従い、秒がない設定でも秒を補完表示。
...
- `Notify threshold`: 自動通知と通常メニューでの時刻調整表示に使うずれのしきい値を変更。`Off` ではNTP基準のアプリ時計は維持し、自動のずれ通知だけを抑止。
- `Startup` -> `Install for this user`: EXEを `%LOCALAPPDATA%\Programs\TrayClockTooltip\` にコピーし、そのコピーを自動起動に登録して起動し、成功通知を表示。
...
- トレイアイコンまたはフローティング時計を `Shift + 右クリック`: 既知のずれが現在の通知しきい値未満の場合でも `Adjust Windows time (admin)` を表示。
...
- 通知しきい値の保持も自動起動登録のコマンドラインを利用します。専用の設定ファイルは作成しません。
